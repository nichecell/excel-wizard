...
class SheetSelectorTool {
  ...
  async analyzeFiles() {
    ...
    this.showProgress("Analyzing files...", 0);
    ...
    try {
      for (let i = 0; i < this.uploadedFiles.length; i++) {
        ...
        this.updateProgress(`Analyzing ${file.name}...`, progress);
        ...
        if (sheetCountEl) {
          sheetCountEl.innerHTML = `📄 ${workbook.SheetNames.length} sheets`;
        }
      }
      ...
      this.hideProgress();
      alert("An error occurred while analyzing files: " + error.message);
      console.error(error);
    }
  }
    ...
    this.uploadedFiles.forEach((file) => {
      ...
      fileHeader.innerHTML = `
                ...
                <span style="margin-left: auto; color: var(--text-secondary);">
                    ${workbook.SheetNames.length} sheets
                </span>
            `;
  ...
  async mergeSelectedSheets() {
    ...
    this.showProgress("Merging sheets...", 0);
    ...
    try {
      ...
      this.hideProgress();
      alert("An error occurred while merging sheets: " + error.message);
      console.error(error);
    }
  }
  ...
  displayMergeResult() {
    ...
    resultEl.innerHTML = `
            ...
            <h3 class="merge-success-title">Merge Complete!</h3>
            <div class="merge-details">
                ${sheetCount} sheets have been successfully merged.<br>
  ...
  downloadMergedFile() {
    ...
    try {
      const date = new Date();
      const filename = `SheetMerge_${ExcelWizardApp.formatDate(date)}.xlsx`;
